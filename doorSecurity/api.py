...
class addMember(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            ...
            try:
                informationService = InformationService.objects.get(
...
class getAllMember(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            data = serializer.validated_data
            hash_serial_rasperyPi = data.get('hash_serial_rasperyPi')
            members = Members.objects.filter(rassperySystem__hash_serial_rassperyPi=hash_serial_rasperyPi).order_by(
                'add_date')
            lis = []
...
class getHistory(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            data = serializer.validated_data
            hash_serial_rasperyPi = data.get('hash_serial_rasperyPi')
            historys = history.objects.filter(rassperypiInfo__hash_serial_rassperyPi=hash_serial_rasperyPi).order_by(
                'date')
            lis = []
            for i in historys:
                d = {}
            ...
            try:
                ...
                async_to_sync(channel_layer.group_send)(
                    group_name,
                    {
                        'type': 'sendMassege',
                        'message': json.dumps(
                            {'massege': 'change status requestOpenDoor', 'code': (1011 + status_openDoor)})
                    })
...
class deleteMember(APIView):
    schema = schemas.DeleteMemberSchema()
    permission_classes = (IsAuthenticated,)
    authentication_classes = (TokenAuthentication,)

    def post(self, request, *args, **kwargs):
        serializer = serializers.deleteMember(data=request.data)
        if serializer.is_valid():
            data = serializer.validated_data
            id_member = data.get('id_member')
            hash_serial_rasperyPi = data.get('hash_serial_rasperyPi')
            print(id_member, hash_serial_rasperyPi)
            # try:
            #     rassperyInfo = RassperySystem.objects.get(hash_serial_rassperyPi=hash_serial_rasperyPi,
            #                                               profile__user=request.user)
            # except:
